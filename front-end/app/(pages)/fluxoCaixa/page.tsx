...
export default function FluxoCaixaPage() {
  ...
  // Buscar dados reais do backend e mapear para o formato do frontend
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Mapeia id_filial para nome dinamicamente
        ...
        // Atualiza estado de filiais para uso nos componentes
        setFiliais([
          "Academia PowerFit",
          ...filiaisData.map((f: any) => f.nome_filial || f.nome),
        ]);

        // Inicializa selectedFilial e selectedFiliaisForComparison se ainda não definidos
        if (!selectedFilial && filiaisData.length > 0) {
          setSelectedFilial(filiaisData[0].nome_filial || filiaisData[0].nome);
        }
        if (
          selectedFiliaisForComparison.length === 0 &&
          filiaisData.length > 0
        ) {
          setSelectedFiliaisForComparison(
            filiaisData.slice(0, 5).map((f: any) => f.nome_filial || f.nome)
          );
        }
  ...
  // Gerar dados dos gráficos a partir das transações reais
  const allCashFlowData = useMemo(() => {
    return MONTHS.map((month) => {
      const entrada = transactions
        .filter((t: Transaction) => t.month === month && t.type === "entrada")
  ...
  const entradaDataByFilial = useMemo(() => {
    const result: Record<string, { month: string; entrada: number }[]> = {};
    filiais.forEach((filial) => {
      result[filial] = MONTHS.map((month) => {
        const entrada = transactions
          .filter(
            (t: Transaction) =>
              t.month === month && t.filial === filial && t.type === "entrada"
          )
          .reduce((sum: number, t: Transaction) => sum + t.value, 0);
        return { month, entrada };
      });
  ...
  const filteredGeneralData = useMemo(() => {
    return allCashFlowData.filter((m) => selectedMonths.includes(m.month));
  }, [selectedMonths, allCashFlowData]);
  ...
  }, [selectedMonths, selectedFilial, filialViewMode, entradaDataByFilial]);
